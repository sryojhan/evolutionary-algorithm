...
import Common.CalcularFitness.DecodificationData;
...
public class Mainframe extends JFrame {
	...
	private JTextField genSizeTF;
	private JTextField wrappingTF;
	...
	public Mainframe() {
		...
		super("P3B PE - Yojhan Steven García Peña");
		instance = this;
		...
		crossTypeComboBox.setModel(new DefaultComboBoxModel(new String[] { "Monopunto", "Uniforme"
		...
		mutationTypeComboBox
				.setModel(new DefaultComboBoxModel(new String[] { "Uniforme", "No uniforme" }));
		...
		JLabel wrapsLabel = new JLabel("Numero de wraps");
		wrapsLabel.setHorizontalAlignment(SwingConstants.LEFT);
		wrappingTF = new JTextField();
		wrappingTF.setText("12");

		problemPanel.add(wrapsLabel);
		problemPanel.add(wrappingTF);

		JLabel geneSizeLabel = new JLabel("Tamaño del gen");
		geneSizeLabel.setHorizontalAlignment(SwingConstants.LEFT);
		genSizeTF = new JTextField();
		genSizeTF.setText("10");

		problemPanel.add(geneSizeLabel);
		problemPanel.add(genSizeTF);
		...
		executeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int genSize = Integer.parseInt(genSizeTF.getText().replace(".", ""));
				int wraps =Integer.parseInt(wrappingTF.getText().replace(".", ""));
				...
				Algoritmo algoritmo = EjecutarAlgoritmo(tamPoblacion, nGeneraciones, elitism, probCruce, probMutacion,
						selectionType, crossingType, mutationType, tamTorneo, escalado, wraps, genSize, n);

				PlotAlgoritmo(algoritmo, showAll.isSelected(), false, wraps);
	...
	private Algoritmo EjecutarAlgoritmo(int tamPoblacion, int nGeneraciones, double elitism, double probCruce,
			...
			boolean escaladoLineal, int wraps, int genSize, int repeticiones) {

		InformacionAlgoritmo info = new InformacionAlgoritmo(false, wraps);

		info.fitnessFunction = (input, wrap) -> {

			return CalcularFitness.CalculateFitness(input, wrap);
	...
	private void PlotAlgoritmo(Algoritmo algoritmo, boolean showAllFitness, boolean clearPrevious, int maxWraps) {

		int[] mejorIndividuo = algoritmo.mejor;
		
		String ecuacionSolucion = CalcularFitness.Decodificar(mejorIndividuo, new DecodificationData(mejorIndividuo.length, maxWraps), true);
		...
		for (int i = 0; i < numeroPuntos; i++) {
			...
			ejeYCalculado[i] = CalcularFitness.CalculateFitness(x, mejorIndividuo, new DecodificationData(mejorIndividuo.length, maxWraps), true);
		}
			...
			for (double m = mutacionMin; m < mutacionMax; m += incMutacion) {
				for (double c = cruceMin; c < cruceMax; c += incCruce) {
					...
					Algoritmo algoritmo = EjecutarAlgoritmo(p, nGeneraciones, elitism, c, m, selectionType,
							crossingType, mutationType, tamTorneo, escalado, wraps, genSize, n);
					...
					if (mejor == null || nuevoResultado < mejor.MejorResultado()) {
						...
						PlotAlgoritmo(mejor, showAll.isSelected(), true, wraps);
