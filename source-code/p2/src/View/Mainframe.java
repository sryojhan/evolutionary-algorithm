...
import Common.DistanciaCiudades;
...
public class Mainframe extends JFrame {
	...
	public Mainframe() {
		...
		super("P2 PE - Yojhan Steven García Peña");
		instance = this;
		...
		crossTypeComboBox.setModel(new DefaultComboBoxModel(
				new String[] { "PMX", "OX", "OX-PP", "CX", "ERX", "CO", "Yojhan"
		...
		mutationTypeComboBox.setModel(new DefaultComboBoxModel(new String[] { "Insercion", "Intercambio", "Inversion", "Heuristica", "Yojhan"}));
			...
			public void actionPerformed(ActionEvent e) {
				...
				Runnable myRunnable = new Runnable() {
		            public void run() {
		                ...
		                instance.AnalisisParametros(nGeneraciones, elitism, selectionType, crossingType, mutationType, tamTorneo); 
		...
		JLabel mejorDistanciaLabel = new JLabel("Mejor distancia");
		...
		gl_westPanel.setHorizontalGroup(gl_westPanel.createParallelGroup(Alignment.TRAILING).addGroup(Alignment.LEADING,
				gl_westPanel
						.createSequentialGroup().addGap(10).addGroup(gl_westPanel.createParallelGroup(Alignment.LEADING)
								...
								.addComponent(analisisPanel, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
								.addComponent(panelResultados, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
								)
						.addGap(10)));
		gl_westPanel.setVerticalGroup(gl_westPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_westPanel.createSequentialGroup().addGap(1)
						...
						.addComponent(analisisPanel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addPreferredGap(ComponentPlacement.RELATED)
						.addComponent(panelResultados, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						));
		...
		gl_solutionPanel.setHorizontalGroup(gl_solutionPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_solutionPanel.createSequentialGroup().addContainerGap().addComponent(solutionLabel)
						.addGap(18)
						.addComponent(recorridoTF, 50, 50, 100)
						...
						gl_solutionPanel.createSequentialGroup().addContainerGap()
								.addGroup(gl_solutionPanel.createParallelGroup(Alignment.TRAILING)
										.addComponent(solutionTF, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
												GroupLayout.PREFERRED_SIZE)
										.addGap(10)
										.addComponent(recorridoTF, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
												GroupLayout.PREFERRED_SIZE)
		...
		executeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Algoritmo algoritmo = EjecutarAlgoritmo(tamPoblacion, nGeneraciones, elitism, probCruce, probMutacion, selectionType, crossingType, mutationType, tamTorneo, escaladoLineal.isSelected());
	...
	private Algoritmo EjecutarAlgoritmo(
			int tamPoblacion, int nGeneraciones, double elitism, double probCruce, double probMutacion, 
			int selectionType, int crossingType, int mutationType, int tamTorneo, boolean escaladoLineal)
	{
		
		InformacionAlgoritmo info = null;

		info = new InformacionAlgoritmo(DistanciaCiudades.NumeroCiudades(), false);
	...
	private void PlotAlgoritmo(Algoritmo algoritmo, boolean showAllFitness, boolean clearPrevious) {
		...
		String recorridoSolucionStr = "Madrid-> ";

		int[] recorridoSolucion = algoritmo.PuntoSolucion();

		for (int i = 0; i < recorridoSolucion.length; i++) {

			recorridoSolucionStr += DistanciaCiudades.NombreCiudad(recorridoSolucion[i]) + "-> ";
		}
		
		
		recorridoSolucionStr += "Madrid";
		

		recorridoTF.setText("" + algoritmo.MejorResultado());
		solutionTF.setText(recorridoSolucionStr);
	...
	public void AnalisisParametros
	(
			int nGeneraciones, double elitism, int selectionType, int crossingType, int mutationType, int tamTorneo
	) {
		...
		for(int p = poblacionMin; p < poblacionMax; p += incPoblacion) {
			for(double m = mutacionMin; m < mutacionMax; m += incMutacion) {
				for(double c = cruceMin; c < cruceMax; c += incCruce) {
					...
					Algoritmo algoritmo = EjecutarAlgoritmo(p, nGeneraciones, elitism, c, m, selectionType, crossingType, mutationType, tamTorneo, escalado);
					...
					if(mejor == null || nuevoResultado < mejor.MejorResultado()) {
						...
						try { //Hay que poner un pequeño sleep para evitar errores en el hilo
							Thread.sleep(10);
