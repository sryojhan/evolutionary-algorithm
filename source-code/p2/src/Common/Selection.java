...
public class Selection {
	...
	private static double RankingIProbability(double Beta, int i, int n) {
		
		
		double value = Beta - 2* (Beta - 1) * (i) / (n -1);;
	...
	public static Individuo[] Ranking(Individuo[] poblacion, double Beta) {
		...
		for(int i = 0; i < size; i++) {
			...
			probabilities[i] = RankingIProbability(Beta, i, size);
		}
		...
		for(int i = 0; i < size; i++) {
			...
			seleccion[i] = ordenado[idx].Clone();
